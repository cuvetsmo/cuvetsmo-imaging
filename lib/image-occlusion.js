...
/**
 * Persist `arr` to localStorage. Returns a status object so callers can
 * surface quota-exceeded / storage-unavailable as a real user error rather
 * than silently dropping data.
 *
 * Return shape:
 *   { ok: true }                                     — saved
 *   { ok: false, reason: 'storage-unavailable' }    — SSR or private mode
 *   { ok: false, reason: 'serialization-failed' }   — JSON.stringify threw
 *   { ok: false, reason: 'QuotaExceededError' }     — quota exceeded even
 *                                                     after LRU evicting all
 *                                                     but the newest deck
 *
 * On QuotaExceededError we still attempt LRU eviction (decks can be 100-300
 * KB each so a single oversized upload can blow the 5 MB quota). Only if
 * even the most-recent single deck doesn't fit do we report failure.
 */
function writeRaw(arr) {
  ...
  try {
    ...
    // Circular reference or similar — defensive; shouldn't happen with
    // normalized decks but better than throwing and looking like success.
  ...
  try {
    ...
    while (fallback.length > 1) {
      ...
      try {
        ...
        return { ok: true, evicted: arr.length - fallback.length };
      } catch {
        /* keep evicting */
      }
    }
    // Could not fit even the newest single deck — caller must surface this.
 ...
 * caller passes a deck without one.
 *
 ...
 *
 * Return shape:
 *   { ok: true,  deck }                              — saved successfully
 *   { ok: false, reason: 'invalid-input' }           — missing/bad payload
 *   { ok: false, reason: 'QuotaExceededError' }      — localStorage full
 *   { ok: false, reason: 'storage-unavailable' }     — SSR / private mode
 *   { ok: false, reason: 'serialization-failed' }    — defensive
 *
 * Callers MUST check `result.ok` before treating the save as committed.
 * Returning the deck unconditionally (old behavior) hid quota errors and
 * caused silent data loss on devices with full localStorage.
 */
export function saveDeck(deck) {
  if (!deck || typeof deck !== 'object') {
    return { ok: false, reason: 'invalid-input' };
  }
  if (typeof deck.imageDataUrl !== 'string' || !deck.imageDataUrl) {
    return { ok: false, reason: 'invalid-input' };
  }
  ...
  const status = writeRaw(next);
  if (!status.ok) {
    // Don't notifyChange — nothing actually persisted.
    return { ok: false, reason: status.reason };
  }
  notifyChange();
  return { ok: true, deck: normalized, evicted: status.evicted };
}

/**
 * Touch lastOpened without modifying other fields. Best-effort — if
 * localStorage is unavailable or full we silently skip; this is read-side
 * bookkeeping and a missed touch only affects LRU ordering, not data.
 */
...
/**
 * Remove a deck by id. Delete strictly shrinks the payload so quota
 * failure is implausible; we still propagate `ok` for completeness.
 *
 * Return shape:
 *   { ok: true }                                  — deleted (or no-op match)
 *   { ok: false, reason: '...' }                  — write failed
 */
export function deleteDeck(deckId) {
  const list = readRaw();
  const next = list.filter((d) => d.id !== deckId);
  if (next.length === list.length) return { ok: true };
  const status = writeRaw(next);
